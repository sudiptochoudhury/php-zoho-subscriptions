 ...
 * @method    array    addNewSubscriptionHostedpages(array $parameters)    Creating a hosted page for a new
 *            subscription for an existing customer
 * @method    array    addNewCustomerNewSubscriptionHostedpages(array $parameters)    Creating a hosted page for a new
 *            subscription for a new customer
 * @method    array    addUpdateCardHostedpages(array $parameters)    Creating a hosted page for updating a card linked
 *            to a specific subscription
 * @method    array    addOnetimeAddonHostedpages(array $parameters)    Creating a hosted page for onetime addon
 *            purchase
 * @method    array    addUpdateSubscriptionHostedpages(array $parameters)    Creating a hosted page for updating an
 ...
 * @method    array    disableCustomerPaymentReminder(array $parameters)    Disabling payment reminder for a particular
 *            customer
 ...
 * @method    array    enableCustomerPaymentReminder(array $parameters)    Enabling payment reminder for a particular
 *            customer
 * @method    array    deleteCustomerAddress(array $parameters)    Delete an existing customer's address.
 * @method    array    deleteCustomerPaypalAccount(array $parameters)    To delete a particular PayPal account of a
 *            particular customer
 ...
 * @method    array    addCreditnoteCustomFields(array $parameters)    Updating custom fields for a credit note You can
 *            update custom fields only if they are configured
 ...
 * @method    array    deletePlan(array $parameters)    Delete an existing plan. A plan can only be deleted if it has
 *            no transactions associated with it.
 * @method    array    getPlans(array $parameters)    Listing all the plans
 * @method    array    addSubscriptionNotes(array $parameters)    Notes related to a particular subscription can be
 *            added by any user at any time
 * @method    array    reactivateSubscription(array $parameters)    Reactivating a subscription Only a subscription
 *            which is in
 * @method    array    deleteSubscriptionNote(array $parameters)    Delete a specific note associated to a particular
 *            subscription
 * @method    array    addSubscription(array $parameters)    Creating an online subscription for an already existing
 *            customer
 * @method    array    cancelSubscriptions(array $parameters)    Bulk cancelling multiple subscriptions
 * @method    array    addSubscriptionLineItem(array $parameters)    Add/Edit description to a particular plan in the
 *            line items list
 * @method    array    deleteSubscriptionCard(array $parameters)    Delete a card associated with the subscription.
 *            Once removed, the subscription will become an offline subscription.
 * @method    array    updateSubscription(array $parameters)    Updating the details of a particular subscription
 * @method    array    addSubscriptionBankAccount(array $parameters)    Associate an existing bank account/Update to an
 *            existing bank account with a subscription
 ...
 * @method    array    addSubscriptionOnetimeAddon(array $parameters)    Include a one-time addon to a particular
 *            subscription
 * @method    array    addOfflineSubscription(array $parameters)    Creating an offline subscription for an already
 *            existing customer
 ...
 * @method    array    getSubscriptionScheduledChanges(array $parameters)    Retrieving the scheduled changes of a
 *            particular subscription
 * @method    array    addSubscriptionCard(array $parameters)    Associate/Update card with an existing card for a
 *            particular subscription
 * @method    array    getSubscriptionRecentActivities(array $parameters)    Retrieving all activities of a particular
 *            subscription
 * @method    array    getSubscriptionNotes(array $parameters)    Retrieving notes of a particular subscription
 * @method    array    deleteSubscriptionScheduledChanges(array $parameters)    Dropping the scheduled changes of a
 *            particular subscription
 * @method    array    addSubscriptionCustomFields(array $parameters)    Update custome fields for a particular
 *            subscription
 * @method    array    addSubscriptionNewBankAccount(array $parameters)    Associate/Update bank account with a new
 *            bank account for a particular subscription
 * @method    array    addSubscriptionsCompute(array $parameters)    Preview amount of the subscription and invoice.
 *            This can be called while both at time of creating a subscription/updating a subscription. For compute
 *            during update, you will have to pass the subscription_id node in the JSON body
 * @method    array    setSubscriptionOffline(array $parameters)    Change a particular subscription to offline mode
 * @method    array    updateSubscriptionCard(array $parameters)    Associate/Update card with a new card for a
 *            particular subscription
 * @method    array    addSubscriptionOnetimeCharge(array $parameters)    Charge a one-time amount for a subscription
 * @method    array    addNewCustomerSubscription(array $parameters)    Creating a subscription for a new customer This
 *            works as a
 * @method    array    cancelSubscriptionImmediately(array $parameters)    Cancelling the subscription immediately
 * @method    array    addSubscriptionPaypalAccount(array $parameters)    Associate/Update PayPal account with an
 *            existing PayPal account for a particular subscription
 * @method    array    postponeSubscription(array $parameters)    Renewal date refers to the billing date of the
 *            subsequent term. You can postpone date of renewal of the subscription by specifying an appropriate date
 *            on which the customer should be billed. No prorated charges will be applied. Billing date for the
 *            subscription will be replaced by the date which you provide
 * @method    array    deleteSubscriptionBankAccount(array $parameters)    Delete a bank account associated with the
 *            subscription. Once removed, the subscription will become an offline subscription.
 * @method    array    addSubscriptionContactPersons(array $parameters)    Associate an existing contact person with a
 *            particular subscription
 * @method    array    addSubscriptionReference(array $parameters)    Update reference id to easily identify and keep
 *            track of your subscription
 * @method    array    getCustomersSubscriptions(array $parameters)    Listing all the subscriptions
 * @method    array    addSubscriptionAddonLineItem(array $parameters)    Add/Edit description to a particular add-on
 ...
 * @method    array    cancelSubscriptionAtEnd(array $parameters)    Cancelling the subscription at the end of term of
 *            subscription
 * @method    array    addSubscriptionSalesPerson(array $parameters)    Update sales person associated with a
 *            particular subscription
 * @method    array    getCustomerCard(array $parameters)    Retrieving details of a particular card for a particular
 *            customer
 ...
 * @method    array    updateCustomerCard(array $parameters)    Editing the existing Credit card information for a
 *            customer
 ...
 * @method    array    addPaymentRefunds(array $parameters)    A new credit note is created for the amount to be
 *            refund. Refund is then made for the credit note.
 * @method    array    deletePayment(array $parameters)    Delete an existing payment.
 * @method    array    addProductCustomFields(array $parameters)    Updating custom fields for a payment You can update
 *            custom fields only if they are configured
 * @method    array    updatePayment(array $parameters)    Update an existing new payment
 * @method    array    getWebhook(array $parameters)    Retrieving details of a particular webhooks that has been
 *            triggered
 * @method    array    getWebhooks()    Retrieving details of all webhooks triggered
 * @method    array    updateCustomerContactPerson(array $parameters)    Updating a particular contact person for a
 *            particular customer
 * @method    array    addCustomerContactPersons(array $parameters)    Creating a contact person for an existing
 *            customer
 * @method    array    getCustomerContactPersons(array $parameters)    List of all contact persons for a particular
 *            customer
 * @method    array    addCustomerPrimaryContactPerson(array $parameters)    Mark a contact person as primary contact
 *            for a customer
 * @method    array    deleteCustomerContactPerson(array $parameters)    Delete an existing contact person.
 * @method    array    getCustomerContactPerson(array $parameters)    Retrieving details of a particular contact person
 *            for a particular customer
 * @method    array    deleteCustomerBankAccount(array $parameters)    To delete a particular Bank account of a
 *            particular customer
 * @method    array    updateCustomerBankAccount(array $parameters)    Editing the existing Bank Account information
 *            for a particular customer
 * @method    array    addCustomerBankAccounts(array $parameters)    Creating a new bank account for a particular
 *            customer
 * @method    array    getCustomerBankAccount(array $parameters)    Retrieving details of a particular bank account for
 ...
 * @method    array    enableInvoicePaymentReminder(array $parameters)    Enabling payment reminder for a particular
 *            invoice
 ...
 * @method    array    getInvoiceRecentActivities(array $parameters)    Retrieving recent activities of a particular
 *            invoice
 ...
 * @method    array    addInvoiceCustomFields(array $parameters)    Updating custom fields for an invoice You can
 *            update custom fields only if they are configured
 * @method    array    disableInvoicePaymentReminder(array $parameters)    Stop all payment reminder for a particular
 *            invoice
 * @method    array    getBankAccountPendingInvoices(array $parameters)    Retrieving ACH pending invoices of a
 ...
 * @method    array    deleteInvoiceCreditsApplied(array $parameters)    Deleting an existing credit applied to the
 *            invoice.
 * @method    array    deleteInvoice(array $parameters)    Deleting an existing invoice.
 * @method    array    addBankAccountCharge(array $parameters)    Charge a customer for an invoice with an existing
 *            bank account
 * @method    array    setInvoicePaymentDate(array $parameters)    Update expected payment date fop the invoice.
 *            Reminders won't be sent till the date specified
